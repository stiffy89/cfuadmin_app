...
const ContactsCFUPhoneBook = () => {
    ...
    return (
        <ScrollView style={{paddingBottom: 40, backgroundColor: theme.colors.background}}>
            <Searchbar style={{marginVertical: 20, marginHorizontal: 20, backgroundColor: theme.colors.surfaceVariant}} value={searchValue} maxFontSizeMultiplier={1.5} placeholder='Search Suburbs' onChangeText={(text) => {
                                                ...
                                                return (
                                                    <React.Fragment key={`suburb_${letter}_${ii}`}>
                                                        ...
                                                        <List.Item onPress={() => {
                                                            ...
                                                            }} right={() => <View style={{flexDirection: 'row', alignItems: 'center'}}><CustomText style={{marginRight: 20}}>{suburb.ContactsCount}</CustomText><LucideIcons.ChevronRight color={theme.colors.primary}/></View>} style={{marginLeft: 20}} key={'item_' + ii} /*title={suburb.SuburbName}*/ title={<View style={{flex: 1}}><CustomText variant='titleMedium'>{suburb.SuburbName}</CustomText></View>} />
                                                        <Divider/>
                                                    </React.Fragment>
                                                )
                                            })
